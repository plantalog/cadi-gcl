...
{
	// Be sure that CS and SDI in Hi state
	GPIO_InitTypeDef GPIO_InitStructure;
	SPI_InitTypeDef SPI_InitStructure;

	RCC_AHB1PeriphClockCmd(MMC_CS_CLOCK, ENABLE);
	RCC_AHB1PeriphClockCmd(MMC_DATA_CLOCK, ENABLE);
	RCC_APB1PeriphClockCmd(MMC_SPI_CLOCK, ENABLE);


	GPIO_PinAFConfig(MMC_SPI_PORT, MMC_SCK_AF, MMC_AF); 		 // SCK
	GPIO_PinAFConfig(MMC_SPI_PORT, MMC_MOSI_AF, MMC_AF);		 // MISO
	GPIO_PinAFConfig(MMC_SPI_PORT, MMC_MISO_AF, MMC_AF);	 	 // MOSI
	...
	GPIO_Init(MMC_SPI_PORT, &GPIO_InitStructure);
	...
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_128;	// max 400khz for initialization
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
	//SPI_InitStructure.SPI_CRCPolynomial = 7;
	SPI_InitStructure.SPI_Mode = SPI_Mode_Master;
