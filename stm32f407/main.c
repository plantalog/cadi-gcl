...
// SD card defines
#define GO_IDLE_STATE            0              //ÐŸÑ€Ð¾Ð³Ñ€Ð°Ð¼Ð¼Ð½Ð°Ñ� Ð¿ÐµÑ€ÐµÐ·Ð°Ð³Ñ€ÑƒÐ·ÐºÐ°
#define SEND_IF_COND             8              //Ð”Ð»Ñ� SDC V2 - Ð¿Ñ€Ð¾Ð²ÐµÑ€ÐºÐ° Ð´Ð¸Ð°Ð¿Ð°Ð·Ð¾Ð½Ð° Ð½Ð°Ð¿Ñ€Ñ�Ð¶ÐµÐ½Ð¸Ð¹
#define READ_SINGLE_BLOCK        17             //Ð§Ñ‚ÐµÐ½Ð¸Ðµ ÑƒÐºÐ°Ð·Ð°Ð½Ð½Ð¾Ð³Ð¾ Ð±Ð»Ð¾ÐºÐ° Ð´Ð°Ð½Ð½Ñ‹Ñ…
#define WRITE_SINGLE_BLOCK       24             //Ð—Ð°Ð¿Ð¸Ñ�ÑŒ ÑƒÐºÐ°Ð·Ð°Ð½Ð½Ð¾Ð³Ð¾ Ð±Ð»Ð¾ÐºÐ° Ð´Ð°Ð½Ð½Ñ‹Ñ…
#define SD_SEND_OP_COND	         41             //Ð�Ð°Ñ‡Ð°Ð»Ð¾ Ð¿Ñ€Ð¾Ñ†ÐµÑ�Ñ�Ð° Ð¸Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð°Ñ†Ð¸Ð¸
#define APP_CMD			 55             //Ð“Ð»Ð°Ð²Ð½Ð°Ñ� ÐºÐ¾Ð¼Ð°Ð½Ð´Ð° Ð¸Ð· ACMD <n> ÐºÐ¾Ð¼Ð°Ð½Ð´
#define READ_OCR		 58             //Ð§Ñ‚ÐµÐ½Ð¸Ðµ Ñ€ÐµÐ³Ð¸Ñ�Ñ‚Ñ€Ð° OCR
//Ð¼Ð°ÐºÑ€Ð¾Ð¾Ð¿Ñ€ÐµÐ´ÐµÐ»ÐµÐ½Ð¸Ñ� Ð´Ð»Ñ� ÑƒÐ¿Ñ€Ð°Ð²Ð»ÐµÐ½Ð¸Ñ� Ð²Ñ‹Ð²Ð¾Ð´Ð¾Ð¼ SS
...
//Ð³Ð»Ð¾Ð±Ð°Ð»ÑŒÐ½Ð°Ñ� Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½Ð°Ñ� Ð´Ð»Ñ� Ð¾Ð¿Ñ€ÐµÐ´ÐµÐ»ÐµÐ½Ð¸Ñ� Ñ‚Ð¸Ð¿Ð° ÐºÐ°Ñ€Ñ‚Ñ‹
uint8_t  SDHC;
static FIL cadizlog;                  //Ñ„Ð°Ð¹Ð»Ð¾Ð²Ñ‹Ð¹ Ð¾Ð±ÑŠÐµÐºÑ‚
...
uint8_t no_sd;
FATFS   fs;                   //Ñ�Ñ‚Ñ€ÑƒÐºÑ‚ÑƒÑ€Ð°, Ñ� ÐºÐ¾Ñ‚Ð¾Ñ€Ð¾Ð¹ Ð±ÑƒÐ´ÐµÐ¼ Ñ�Ð²Ñ�Ð·Ñ‹Ð²Ð°Ñ‚ÑŒ Ð´Ð¸Ñ�Ðº
uint16_t VarValue = 0;
uint16_t ph_seven=0;
uint32_t ph0=0;
uint16_t ph4=0;
...
void buttonsInit(void) {
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN; // Ð Ð°Ð·Ñ€ÐµÑˆÐ¸Ñ‚ÑŒ Ñ‚Ð°ÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ðµ Ð¿Ð¾Ñ€Ñ‚Ð° PORTA
	//ÐšÐ¾Ð½Ñ„Ð¸Ð³ÑƒÑ€Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ðµ PORTA.0 - 3 - analog inputs
	GPIOA->MODER |= (GPIO_MODER_MODER0 << (0 * 2)); //ÐžÑ‡Ð¸Ñ�Ñ‚Ð¸Ñ‚ÑŒ Ð±Ð¸Ñ‚Ñ‹ MODE
	GPIOA->MODER |= (GPIO_MODER_MODER0 << (1 * 2)); //ÐžÑ‡Ð¸Ñ�Ñ‚Ð¸Ñ‚ÑŒ Ð±Ð¸Ñ‚Ñ‹ MODE
	GPIOA->MODER |= (GPIO_MODER_MODER0 << (2 * 2)); //ÐžÑ‡Ð¸Ñ�Ñ‚Ð¸Ñ‚ÑŒ Ð±Ð¸Ñ‚Ñ‹ MODE
	GPIOA->MODER |= (GPIO_MODER_MODER0 << (3 * 2)); //ÐžÑ‡Ð¸Ñ�Ñ‚Ð¸Ñ‚ÑŒ Ð±Ð¸Ñ‚Ñ‹ MODE

	RCC->APB2ENR |= RCC_APB2ENR_ADC1EN; //Ð¿Ð¾Ð´Ð°ÐµÐ¼ Ñ‚Ð°ÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ðµ Ð�Ð¦ÐŸ
	ADC1->CR1 = 0; //
	ADC1->CR2 = 0; //
	ADC1->JSQR = 0;

	ADC1->CR2 = ADC_CR2_JEXTSEL; //Ð²Ñ‹Ð±Ñ€Ð°Ñ‚ÑŒ Ð¸Ñ�Ñ‚Ð¾Ñ‡Ð½Ð¸ÐºÐ¾Ð¼ Ð·Ð°Ð¿ÑƒÑ�ÐºÐ° Ñ€Ð°Ð·Ñ€Ñ�Ð´ JSWSTART
	ADC1->CR2 |= ADC_CR2_CONT; //Ñ€ÐµÐ¶Ð¸Ð¼ Ð½ÐµÐ¿Ñ€ÐµÑ€Ñ‹Ð²Ð½Ð¾Ð³Ð¾ Ð¿Ñ€ÐµÐ¾Ð±Ñ€Ð°Ð·Ð¾Ð²Ð°Ð½Ð¸Ñ�
	ADC1->CR1 |= ADC_CR1_SCAN; //Ñ€ÐµÐ¶Ð¸Ð¼ Ñ�ÐºÐ°Ð½Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ñ� (Ñ‚.Ðµ. Ð½ÐµÑ�ÐºÐ¾Ð»ÑŒÐºÐ¾ ÐºÐ°Ð½Ð°Ð»Ð¾Ð²)
	ADC1->CR1 |= ADC_CR1_JAUTO; //Ð°Ð²Ñ‚Ð¾Ð¼Ð°Ñ‚. Ð·Ð°Ð¿ÑƒÑ�Ðº Ð¸Ð½Ð¶ÐµÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð½Ð¾Ð¹ Ð³Ñ€ÑƒÐ¿Ð¿Ñ‹
...
void init_gpio(void){	// do we need it?
	 GPIO_InitTypeDef gpio_cfg,GPIO_InitStructure;
	  ...
	  // vyvod ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨trigÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¨ signala
...
int main(void)
{
	...
	dht_init();
	buttonsInit();
int	del=1000*SPEED_K; while (del--){}
	uint8_t i=0, res=0;
	uint16_t i2;
	for (i=0; i<16; i++) {
		LCDLine1[i] = 32;
	...
	uint16_t	tmpvar=0;
	Lcd_goto(0,0);
	Lcd_write_str("sd test");

	no_sd = disk_initialize(0);
	no_sd = f_mount(0, &fs);              // mount file system

	char FileName[]="0:CADIZLOG";  //Ð¸Ð¼Ñ� Ð»Ð¾Ð³ Ñ„Ð°Ð¹Ð»Ð°
	res = f_open(&cadizlog, FileName, FA_WRITE | FA_READ);
	if(res) {
		res = f_open(&cadizlog, FileName, FA_WRITE | FA_CREATE_ALWAYS);
		if(res) {
		  return res;    //ÐµÑ�Ð»Ð¸ Ð¿Ñ€Ð¾Ð¸Ð·Ð¾ÑˆÐ»Ð° Ð¾ÑˆÐ¸Ð±ÐºÐ°
		}
	}    //ÐµÑ�Ð»Ð¸ Ð¿Ñ€Ð¾Ð¸Ð·Ð¾ÑˆÐ»Ð° Ð¾ÑˆÐ¸Ð±ÐºÐ°
	f_lseek(&cadizlog, cadizlog.fsize);

		string2log("System started\n", 15);
		del=3000*SPEED_K; while (del--){}
		string2log("System startet\n", 15);
		del=3000*SPEED_K; while (del--){}
...
//	void vTaskProgramRunner(void *pvParameters)
	...
	xTaskCreate( vTaskLCDdraw, ( signed char * ) "LCD", configMINIMAL_STACK_SIZE, NULL, 2, ( xTaskHandle * ) NULL);
...
FRESULT string2log(char* str, int amount){
	uint8_t res=0;               //Ð´Ð»Ñ� Ð²Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°ÐµÐ¼Ð¾Ð³Ð¾ Ñ„ÑƒÐ½ÐºÑ†Ð¸Ñ�Ð¼Ð¸ Ñ€ÐµÐ·ÑƒÐ»ÑŒÑ‚Ð°Ñ‚Ð°
	UINT len;                  //Ð´Ð»Ñ� Ñ…Ñ€Ð°Ð½ÐµÐ½Ð¸Ñ� ÐºÐ¾Ð»Ð¸Ñ‡ÐµÑ�Ñ‚Ð²Ð° Ñ€ÐµÐ°Ð»ÑŒÐ½Ð¾ Ð·Ð°Ð¿Ð¸Ñ�Ð°Ð½Ð½Ñ‹Ñ… Ð±Ð°Ð¹Ñ‚
...
/*
	...
	Lcd_goto(0,0);
	Lcd_write_str("1:");
	Lcd_write_digit(button_val[0]/100%100);
	Lcd_write_digit(button_val[0]%100);
	Lcd_write_str(";");
	Lcd_write_digit(button_ranges[0]/100%100);
	Lcd_write_digit(button_ranges[0]%100);
	Lcd_write_str("-");
	Lcd_write_digit(button_ranges[1]/100%100);
	Lcd_write_digit(button_ranges[1]%100);
//	Lcd_write_str(";");
	Lcd_goto(1,0);
	Lcd_write_str("2:");
	Lcd_write_digit(button_val[1]/100%100);
	Lcd_write_digit(button_val[1]%100);
	Lcd_write_str(";");
	Lcd_write_digit(button_ranges[2]/100%100);
	Lcd_write_digit(button_ranges[2]%100);
	Lcd_write_str("-");
	Lcd_write_digit(button_ranges[3]/100%100);
	Lcd_write_digit(button_ranges[3]%100);
	Delay_us(80000);
	Lcd_goto(0,0);
	Lcd_write_str("3:");
	Lcd_write_digit(button_val[2]/100%100);
	Lcd_write_digit(button_val[2]%100);
	Lcd_write_str(";");
	Lcd_write_digit(button_ranges[4]/100%100);
	Lcd_write_digit(button_ranges[4]%100);
	Lcd_write_str("-");
	Lcd_write_digit(button_ranges[5]/100%100);
	Lcd_write_digit(button_ranges[5]%100);
//	Lcd_write_str(";");
	Lcd_goto(1,0);
	Lcd_write_str("4:");
	Lcd_write_digit(button_val[3]/100%100);
	Lcd_write_digit(button_val[3]%100);
	Lcd_write_str(";");
	Lcd_write_digit(button_ranges[6]/100%100);
	Lcd_write_digit(button_ranges[6]%100);
	Lcd_write_str("-");
	Lcd_write_digit(button_ranges[7]/100%100);
	Lcd_write_digit(button_ranges[7]%100);
//	Lcd_write_str(";");
	Delay_us(80000);
*/
